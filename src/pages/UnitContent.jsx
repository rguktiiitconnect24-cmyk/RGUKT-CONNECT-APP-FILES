...
const UnitContent = () => {
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        const modulesPromise = withTimeout(
          fetchDynamicModules(unitId),
          8000
        ).catch(() => []);

        const resolveHierarchy = async () => {
          const program = PROGRAMS.find((p) =>
            p.years.some((y) => y.id === yearId)
          );
          ...
          if (!subject && program) {
            try {
              const dynSubjects = await withTimeout(
                fetchDynamicSubjects(
                  program.id,
                  yearId,
                  branchId || null,
        ...
        const resolveUnit = async (subject) => {
          ...
          try {
            if (!unit && subjectId) {
              const dynUnits = await withTimeout(
                fetchDynamicUnits(subjectId),
                8000
              );
        ...
        if (unit) {
          ...
          for (let module of loadedModules) {
            const modulePdf = await resolvePdf(module.pdfUrl);
            if (modulePdf) module.resolvedPdfData = modulePdf;
            if (module.additionalNotes && module.additionalNotes.length > 0) {
              for (let note of module.additionalNotes) {
                const noteData = await resolvePdf(note.url);
                if (noteData) note.resolvedData = noteData;
              }
            }
          ...
          setCurrentUnit(unit);
          setDynamicModules(loadedModules);
  ...
  const hierarchy = [
    searchParams.get("branch")
      ? `Branch_${searchParams.get("branch")}`
      : contextData.program?.label || "General",
  ...
  useEffect(() => {
    const fetchPendingQuiz = async () => {
      if (currentUnit && user && subjectId) {
        ...
        try {
          const data = await getPendingQuiz(
            user.uid,
            uniqueModuleId,
            selectedDifficulty
          );
  ...
  const handleDeletePendingQuiz = async () => {
    if (
      ...
      localStorage.removeItem(
        `quiz_progress_${uniqueModuleId}_${selectedDifficulty}`
      );
      setPendingQuizData(null);
    }
  };
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      ...
      const filteredQuestions = questions.filter(
        (q) => (q.difficulty || "Moderate") === difficulty
      );

      if (filteredQuestions.length === 0) {
        setQuizError(
          `No ${difficulty} questions available for this unit yet. Try another difficulty!`
        );
        setIsGeneratingQuiz(false);
        return;
      }
  ...
  if (loading)
    return <LoadingTransition message="Learning Content Loading" persistent />;
  if (!currentUnit)
    return (
  ...
  return (
    <div className="uc-page-container">
      ...
      {/* Top Navigation Bar */}
      <div
        className="cmp-top-bar"
        style={{
          margin: "1.2rem 16px 0",
          width: "auto",
          padding: "6px 14px",
          minHeight: "48px",
        }}
      >
        ...
        >
          ...
          >
            ...
            >
              {contextData.subject?.label || "Mathematics"}
            </p>
          </div>
          <div
            className="cmp-header-icon"
            style={{
              width: "28px",
              height: "24px",
              marginLeft: "10px",
          ...
          >
            ...
            >
              ...
              <g transform="scale(1.1) translate(-10, -10)">
                ...
                {/* Bottom Book */}
                <path
                  d="M 120 200 L 280 200 L 300 230 L 140 230 Z"
                  className="primary"
                />
                <path
                  d="M 120 200 L 280 200 L 300 230 L 140 230 Z"
                ...
                />
                <path
                  d="M 120 200 L 140 230 L 140 240 L 120 210 Z"
                ...
                />
                <path
                  d="M 140 230 L 300 230 L 300 240 L 140 240 Z"
                  className="white"
                />
                <path
                  d="M 140 230 L 300 230 L 300 240 L 140 240 Z"
                ...
                {/* Middle Book */}
                <path
                  d="M 100 160 L 250 160 L 270 190 L 120 190 Z"
                  className="accent"
                />
                <path
                  d="M 100 160 L 250 160 L 270 190 L 120 190 Z"
                ...
                />
                <path
                  d="M 100 160 L 120 190 L 120 200 L 100 170 Z"
                  className="dark-accent"
                />
                <path
                  d="M 100 160 L 120 190 L 120 200 L 100 170 Z"
                ...
                />
                <path
                  d="M 120 190 L 270 190 L 270 200 L 120 200 Z"
                  className="white"
                />
                <path
                  d="M 120 190 L 270 190 L 270 200 L 120 200 Z"
      ...
      <main className="uc-main">
        {/* Content Area */}
        {currentUnit.pdfUrl ||
        (currentUnit.additionalNotes &&
          currentUnit.additionalNotes.length > 0) ||
        (dynamicModules &&
          dynamicModules.some(
            (m) =>
              m.pdfUrl || (m.additionalNotes && m.additionalNotes.length > 0)
          )) ? (
          <>
            {(currentUnit.pdfUrl ||
              (currentUnit.additionalNotes &&
              ...
              >
                <div className="mb-4">
                  ...
                  <p className="text-sm text-slate-500">
                    Download materials to view them offline
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {currentUnit.pdfUrl && (
                    <PdfDownloadCard
                      label={
                        currentUnit.pdfName ||
                        currentUnit.resolvedPdfData?.fileName ||
                        "Main Notes"
                      }
                      url={
                        currentUnit.resolvedPdfData?.pdfUrl ||
                        currentUnit.resolvedPdfData?.downloadUrl ||
                        currentUnit.resolvedPdfData?.url ||
                        currentUnit.resolvedPdfData?.googleDriveDownloadLink ||
                        currentUnit.pdfUrl
                      }
                  ...
                  {currentUnit.additionalNotes?.map((note, idx) => (
                    <PdfDownloadCard
                      ...
                      url={
                        note.resolvedData?.pdfUrl ||
                        note.resolvedData?.downloadUrl ||
                        note.resolvedData?.url ||
                        note.resolvedData?.googleDriveDownloadLink ||
                        note.url
                      }
                      idPrefix={currentUnit.id}
                      hierarchy={hierarchy}
              ...
              >
                <div
                  ...
                  style={{
                    padding: "16px 20px",
                    background: "#ffffff",
                    borderBottom: isModulesExpanded
                ...
                {isModulesExpanded && (
                  <div style={{ background: "#ffffff" }}>
                    ...
                    <div style={{ padding: "20px" }}>
                      {dynamicModules.map((module, mIdx) => {
                        if (module.id !== selectedModuleId) return null;

                        return (
                          <div key={module.id} className="animate-fade-in">
                            {module.pdfUrl ||
                            (module.additionalNotes &&
                              module.additionalNotes.length > 0) ? (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {module.pdfUrl && (
                                  <PdfDownloadCard
                                    label={
                                      module.pdfName ||
                                      module.resolvedPdfData?.fileName ||
                                      "Module Notes"
                                    }
                                    url={
                                      module.resolvedPdfData?.pdfUrl ||
                                      module.resolvedPdfData?.downloadUrl ||
                                      module.resolvedPdfData?.url ||
                                      module.resolvedPdfData
                                        ?.googleDriveDownloadLink ||
                                      module.pdfUrl
                                    }
                                    idPrefix={module.id || mIdx}
                                    hierarchy={hierarchy}
                                    backendUrl={
                                      module.resolvedPdfData?.backendUrl
                                    }
                                  />
                                )}
                                {module.additionalNotes?.map((note, idx) => (
                                  <PdfDownloadCard
                                    key={idx}
                                    label={
                                      note.resolvedData?.fileName || note.label
                                    }
                                    url={
                                      note.resolvedData?.pdfUrl ||
                                      note.resolvedData?.downloadUrl ||
          ...
          <section className="uc-empty-state">
            <div className="uc-orb-container">
              ...
              <div className="uc-orb-icon">
                <i
                  ...
                  style={{ color: "rgb(255, 46, 17)", fontSize: "40px" }}
                ></i>
              </div>
            </div>
            <div className="uc-empty-texts">
              ...
              <p className="uc-empty-desc">
                Our professors are carefully curating premium notes for this
                unit. Check back soon for deeper insights.
              </p>
            </div>
            <button
            ...
            <div className="uc-quiz-card">
              ...
              <div className="uc-quiz-header">
                ...
                <div>
                  ...
                  <p className="uc-quiz-desc">
                    Challenge yourself with timed MCQs covering all key concepts
                    of {currentUnit?.label || "Unit 1"}.
                  </p>
                </div>
              </div>
              <div className="uc-quiz-actions-row">
                <div className="uc-quiz-difficulty-section">
                  <label className="uc-difficulty-label">
                    Select Difficulty:
                  </label>
              ...
              {pendingQuizData && (
                ...
                >
                  ...
                  >
                    <button
                      onClick={handleDeletePendingQuiz}
                      style={{
                        padding: "8px 14px",
                        borderRadius: "12px",
